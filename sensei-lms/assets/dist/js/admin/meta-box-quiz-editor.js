...
var t,n,i=[];jQuery("#add-question-metadata > table > tbody > tr input.question_type").each((function(){t=jQuery(this).val(),i.push(t)})),
...
// Get current value
var n=parseInt(jQuery("#question_counter").val()),i=n;e=parseInt(e),
// Increment or Decrement
i="-"==t?n-e:n+e,
// Set new value
jQuery("#question_counter").val(i),i>0?jQuery("#no-questions-message").hasClass("hidden")||jQuery("#no-questions-message").addClass("hidden"):jQuery("#no-questions-message").removeClass("hidden");const a=new CustomEvent("sensei-quiz-editor-question-count-updated",{detail:{questions:i}});window.dispatchEvent(a),jQuery.fn.updateQuestionRows()},
/**
   * Save quiz grade type
   *
   * @since 1.3.0
...
e+="&question_number="+n;var i=parseInt(jQuery("#add-multiple-question-count").attr("max"));
...
e&&(t=jQuery("#question_"+i).closest("tr").find(".question_required_fields input[name=question] ").val(),jQuery("#question_"+i).closest("tr").prev().find(".question-title-column").html(t),
// update the grade total
n=jQuery("#question_"+i).closest("tr").find(".question_required_fields input[name=question_grade] ").val(),jQuery("#question_"+i).closest("tr").prev().find(".question-grade-column").html(n),
...
})),e+="&quiz_id="+jQuery("#quiz_id").val(),t=jQuery(this).closest("tr").find("td.question-number span.number").text();var n=jQuery(this).closest("tbody");
// Perform the AJAX call.
...
jQuery("#add-question-metadata > table > tbody > tr").children("td").each((function(){if(jQuery(this).find("span.number").text()==t)
...
/**
   ...
   * @access public
   */
jQuery("#add-question-metadata").on("click","a.question_multiple_delete",(function(){var e="";if(confirm(woo_localized_data.confirm_remove_multiple)){e+="question_id="+jQuery(this).attr("rel"),e+="&quiz_id="+jQuery("#quiz_id").val();var t=jQuery(this).closest("tr").find("td.question-number span.number").text(),n=jQuery(this).closest("tr").find("td.question-number span.total-number").text();
// Perform the AJAX call.
...
/**
   ...
   * @access public
   */
jQuery("#add-new-question").on("click","a.add_existing_save",(function(){var e="",t="",n=0;if(jQuery("#existing-questions").find("input.existing-item").each((function(){if(jQuery(this).is(":checked")){var t=jQuery(this).val();e+=t+",",++n}})),e)return t="questions="+e,t+="&quiz_id="+jQuery("#quiz_id").val(),t+="&question_count="+parseInt(jQuery("#question_counter").val()),
// Perform the AJAX call.
